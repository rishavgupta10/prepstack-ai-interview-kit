...
const ResumeUploadForm = ({ formClose }: ResumeUploadFormProps) => {
    ...
    function handleFile(e: ChangeEvent<HTMLInputElement>) {
        ...
        if (!file) return;
        setResume({ name: file.name, size: file.size, url: URL.createObjectURL(file), file });
        ...
        <div className="mt-4">
            <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm">
                <p className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3 flex items-center gap-1.5">
                    <FileText size={14} className="text-purple-600" /> Upload Resume Document
                </p>
                ...
                {!resume ? (
                    ...
                    >
                        <div className="w-10 h-10 rounded-xl bg-purple-50 border border-purple-200 flex items-center justify-center group-hover:bg-purple-600 group-hover:text-white transition-all shadow-2xs">
                            <Upload size={18} className="text-purple-600 group-hover:text-white transition-colors" />
                        </div>
                        <p className="text-sm font-bold text-slate-900 group-hover:text-purple-600 transition-colors">
                            Click to choose file or drag and drop
                        </p>
                        <p className="text-xs text-slate-400 font-semibold">PDF or DOCX (10 MB maximum size)</p>
                    </div>
                ) : (
                    <div className="border border-slate-200 bg-slate-50/70 rounded-xl p-4 space-y-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-slate-950 truncate">{resume.name}</p>
                                <p className="text-xs font-semibold text-slate-500">{formatSize(resume.size)}</p>
                            </div>
                        </div>

                        <div className="flex gap-2">
                            {[
                                { Icon: Eye, label: "View", onClick: () => window.open(resume.url, "_blank") },
                                { Icon: Download, label: "Download", onClick: () => { const a = document.createElement("a"); a.href = resume.url; a.download = resume.name; a.click(); } },
                                { Icon: RefreshCw, label: "Replace", onClick: () => fileInputRef.current?.click() },
                                { Icon: Trash2, label: "Remove", onClick: () => setResume(null) },
